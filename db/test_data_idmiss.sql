-- ============================================================
-- SAMPLE DATA -- v2, RECONCILED FOR CONTINUITY
-- ============================================================
--
-- v1 had the right shape but the balances were invented independently
-- of the transaction history. This file enforces three invariants:
--
--   INVARIANT 1 (balance from history):
--     For each account:
--       current_balance = opening_balance
--                       + SUM(transactions amount, signed by Plaid convention,
--                             flipped because positive = out)
--                       + SUM(transfers in)
--                       - SUM(transfers out)
--
--   INVARIANT 2 (available from holds):
--     For each account:
--       available_balance = current_balance
--                         - SUM(pending_transactions.amount_hold
--                               WHERE is_settled = FALSE)
--
--   INVARIANT 3 (pending lifecycle):
--     For each row in pending_transactions WHERE is_settled = TRUE:
--       there exists a row in transactions with the same
--       plaid_transaction_id, same account_id, and
--       amount = pending.amount_settled.
--
-- Opening balances are zero everywhere (clean slate, Nov 30, 2025).
-- All final balances are derived from the data below, not chosen.
-- ============================================================
--
...
--
-- ACCOUNT ID MAPPING (assumed):
--   1=Primary Checking   2=Emergency Savings   3=Cash App   4=Quicksilver
--
-- SIGN CONVENTION (Plaid default):
--   transactions.amount > 0  =>  money OUT of account (spend)
--   transactions.amount < 0  =>  money INTO account   (income, refund)
--
-- IF YOU ALREADY RAN v1:
--   The ON CONFLICT clause skips re-inserting accounts. The UPDATEs
--   at the end of this file reset the balances so v2 invariants hold.
--   For transactions/pendings/budgets/transfers, you'll want to
--   truncate the rows from v1 first to avoid PK/UNIQUE collisions:
--
--     DELETE FROM account_transfers WHERE users_id = 1;
--     DELETE FROM budgets WHERE users_id = 1;
--     DELETE FROM pending_transactions WHERE users_id = 1;
--     DELETE FROM transactions WHERE users_id = 1;
--
-- ============================================================


-- ============================================================
-- ACCOUNTS (opening balances zero; updated at end of file)
-- ============================================================
...
VALUES
  (1, 'Primary Checking',   'Chase',       '****1234', 'checking', 0.00, 0.00, 'plaid_acct_chase_chk', NOW() - INTERVAL '2 hours'),
  (1, 'Emergency Savings',  'Chase',       '****5678', 'savings',  0.00, 0.00, 'plaid_acct_chase_sav', NOW() - INTERVAL '2 hours'),
  (1, 'Cash App',           'Cash App',    '****9999', 'savings',  0.00, 0.00, 'plaid_acct_cashapp',   NOW() - INTERVAL '6 hours'),
  (1, 'Quicksilver',        'Capital One', '****4321', 'credit',   0.00, 0.00, 'plaid_acct_capone_cc', NOW() - INTERVAL '2 hours')
...
-- Recurring needs (Primary Checking)
...
-- Wants on Quicksilver (credit card)
...
-- ============================================================
-- TRANSACTIONS -- January 2026 (3-paycheck month, winter light spike)
-- ============================================================
INSERT INTO transactions (users_id, account_id, plaid_transaction_id, merchant_name, category_id, amount, transaction_date, transaction_description) VALUES
...
-- ============================================================
-- TRANSACTIONS -- March 2026 (shoulder season)
-- ============================================================
INSERT INTO transactions (users_id, account_id, plaid_transaction_id, merchant_name, category_id, amount, transaction_date, transaction_description) VALUES
...
-- Note: the two settled pendings (pend_gas_settled, pend_rest_settled)
-- are inserted here too -- the lifecycle requires same plaid_id in
-- both tables.
-- ============================================================
INSERT INTO transactions (users_id, account_id, plaid_transaction_id, merchant_name, category_id, amount, transaction_date, transaction_description) VALUES
...
-- Settled pendings posted to transactions (matched plaid_id in pending_transactions)
(1, 1, 'pend_gas_settled',   'Speedway',    12, 41.80, '2026-06-04 11:00:00-04', 'Gas (settled from pending)'),
(1, 1, 'pend_rest_settled',  'Bru Burger',  19, 36.40, '2026-06-02 13:00:00-04', 'Dinner (settled from pending; tip added)');


-- ============================================================
-- PENDING TRANSACTIONS (all on Primary Checking = debit holds)
-- Three unsettled (sum of holds = $150 = checking balance gap).
-- Two settled, matching the two new rows in transactions above.
-- ============================================================
...
-- Unsettled: gas pump pre-auth ($75 hold for an expected ~$42 fillup)
(1, 1, 12, 'pend_gas_active',     'Speedway',           42.00,  75.00, NULL,  NULL,                       FALSE),
-- Unsettled: restaurant ($25 charge, $30 hold for tip)
(1, 1, 19, 'pend_rest_active',    'Local Restaurant',   25.00,  30.00, NULL,  NULL,                       FALSE),
-- Unsettled anomaly: hold < amount (auth/hold inconsistency)
(1, 1, 22, 'pend_anomaly',        'Online Shop XYZ',    60.00,  45.00, NULL,  NULL,                       FALSE),
-- Settled: gas pump where final ($41.80) < hold ($100). Matches transactions row 'pend_gas_settled'.
(1, 1, 12, 'pend_gas_settled',    'Speedway',           50.00, 100.00, 41.80, '2026-06-04 11:00:00-04',   TRUE),
-- Settled: restaurant where final ($36.40) > pending ($28) by 30% (tip). Matches transactions row 'pend_rest_settled'.
(1, 1, 19, 'pend_rest_settled',   'Bru Burger',         28.00,  35.00, 36.40, '2026-06-02 13:00:00-04',   TRUE);
...
-- Forces invariants 1 and 2 to hold. Run unconditionally so the
-- file is safe to re-run.
-- ============================================================
UPDATE accounts SET current_balance = 9268.78, available_balance = 9118.78
  WHERE plaid_account_id = 'plaid_acct_chase_chk';
UPDATE accounts SET current_balance =  600.00, available_balance =  600.00
  WHERE plaid_account_id = 'plaid_acct_chase_sav';
UPDATE accounts SET current_balance =   50.00, available_balance =   50.00
  WHERE plaid_account_id = 'plaid_acct_cashapp';
UPDATE accounts SET current_balance = 1195.75, available_balance = 1195.75
  WHERE plaid_account_id = 'plaid_acct_capone_cc';


-- ============================================================
-- VERIFICATION QUERIES
-- Each of these should return zero rows or a single zero result.
-- If they return non-zero, the data is inconsistent.
-- ============================================================

-- INVARIANT 1: current_balance == derived flow per account
-- (signed transaction sum is flipped because positive=spend in Plaid convention)
--   SELECT
--     a.account_name,
--     a.current_balance,
--     (SELECT COALESCE(SUM(-amount), 0) FROM transactions t WHERE t.account_id = a.id)
--       + (SELECT COALESCE(SUM(amount_transferred), 0) FROM account_transfers WHERE target_account_id = a.id)
--       - (SELECT COALESCE(SUM(amount_transferred), 0) FROM account_transfers WHERE source_account_id = a.id)
--       AS derived_balance,
--     a.current_balance -
--       ((SELECT COALESCE(SUM(-amount), 0) FROM transactions t WHERE t.account_id = a.id)
--       + (SELECT COALESCE(SUM(amount_transferred), 0) FROM account_transfers WHERE target_account_id = a.id)
...
--   FROM accounts a
--   ORDER BY a.id;
-- For credit accounts the convention is flipped (spend increases what you owe), so
-- Quicksilver's "drift" will appear as 2 * its balance with the above query.
-- Treat credit separately or normalize by account_type.

-- INVARIANT 2: balance gap == sum of unsettled holds
--   SELECT
--     a.account_name,
--     (a.current_balance - a.available_balance) AS gap,
--     COALESCE((SELECT SUM(amount_hold) FROM pending_transactions p
--               WHERE p.account_id = a.id AND p.is_settled = FALSE), 0) AS holds,
--     (a.current_balance - a.available_balance) -
--       COALESCE((SELECT SUM(amount_hold) FROM pending_transactions p
--                 WHERE p.account_id = a.id AND p.is_settled = FALSE), 0) AS drift
--   FROM accounts a
--   ORDER BY a.id;
-- All drift values should be 0.00.

-- INVARIANT 3: settled pendings have matching transactions
--   SELECT p.plaid_transaction_id, p.amount_settled, t.amount,
--          (p.amount_settled - t.amount) AS drift
--   FROM pending_transactions p
--   LEFT JOIN transactions t ON t.plaid_transaction_id = p.plaid_transaction_id
--   WHERE p.is_settled = TRUE;
-- t.amount must equal p.amount_settled for every row.